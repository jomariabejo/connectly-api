...
public class UserService {
    ...
    @Value("${jwt.secret}")
    private String secretKey;  // Inject secret key from properties

    @Value("${jwt.expiration-time}")
    private long jwtExpiration;  // Inject JWT expiration time from properties
    ...
    public AuthResponse registerUser(RegisterRequest registerRequest) {
        if (userRepository.existsByUsername(registerRequest.getUsername())) {
            log.info("Attempt to register with an existing username: {}");
            return new AuthResponse(false, "Username already taken");
        }

        if (userRepository.existsByEmail(registerRequest.getEmail())) {
            log.info("Attempt to register with an existing email: {}");
    ...
    public AuthResponse authenticateUser(LoginRequest loginRequest) {
        ...
        if (userOpt.isEmpty()) {
            log.warning("Invalid login attempt for username/email: {}");
        ...
        if (!passwordEncoder.matches(loginRequest.getPassword(), user.getPassword())) {
            log.warning("Invalid password attempt for user: {}");
            return new AuthResponse(false, "Invalid username or password");
        }
