...
7. [Deploying on Render (PaaS)](#deploying-on-render-paas)
...
The following files have been created for Phase 1:
...
| `build.gradle` | Updated with Actuator, Micrometer, Prometheus |
| `Dockerfile` | Multi-stage build (Gradle → Alpine JRE) |
| `docker-compose.yml` | Orchestrates 3 environments locally |
| `.dockerignore` | Optimizes Docker build context |
| `.env.example` | Template for environment variables |

### Quick Start: Run All 3 Environments Locally

```bash
# 1. Build Docker images (first time, takes ~5-10 minutes)
...
# Output should show 9 containers running:
# - 3 API containers (api-stage, api-demo, api-prod)
# - 3 PostgreSQL containers (postgres-stage, postgres-demo, postgres-prod)
# - 1 Mailpit container (mail testing)
# - 2 Others...
```

### Access Applications

After running `docker-compose up`, access each environment:

| Environment | URL | Database Port |
|-------------|-----|---------------|
| **Stage** | http://localhost:8081/api | 5433 |
| **Demo** | http://localhost:8082/api | 5434 |
| **Prod** | http://localhost:8083/api | 5435 |
...
# Detailed metric on specific endpoint
...
```bash
# Stop and remove containers (keeps volumes for data persistence)
docker-compose down
...
Flyway database migrations run **automatically** on application startup:

```bash
# Migrations location: src/main/resources/db/migration/
# V2__follow_system.sql
# V3__blocking_and_circles.sql
# V4__orders_module.sql
# V5__payments_module.sql
```

**Important:** Each environment (`stage`, `demo`, `prod`) has:
- Separate PostgreSQL container with own database
- Separate volume for data persistence
- Independent Flyway migration track
...
3. Deploy to DigitalOcean Droplet (Phase 4) **or** [Render](RENDER.md) (managed PaaS)
...
For a managed alternative to a self-hosted droplet, use **[RENDER.md](RENDER.md)**. It describes the included [render.yaml](render.yaml) Blueprint (PostgreSQL + Docker web service), required environment variables, health checks (`/api/actuator/health`), and how `DATABASE_URL` is mapped to JDBC for Spring Boot.
